...
class SimplesNacionalService:
    def get(self, faturamento_periodo: float, salarios_valor: float, pro_labore_valor: float) -> SimplesNacional:
        ...
        simples_nacional.set_aliquota(PercentualSimplesNacional.ALIQUOTA.value)
        a_pagar = round(faturamento_periodo * PercentualSimplesNacional.ALIQUOTA.value, 2)
        simples_nacional.set_a_pagar_no_periodo(a_pagar)
        ...
        simples_nacional.set_inss(inss)
        carga = round(a_pagar + inss, 2)
        simples_nacional.set_carga_tributaria_anual(carga)
        simples_nacional.set_percentual_dos_tributos(round(carga / faturamento_periodo, 4))
